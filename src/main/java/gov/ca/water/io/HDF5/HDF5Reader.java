...
public class HDF5Reader {
    ...
    public static void openSVFile(String fileName, String partA, String partF) {
        boolean openSuccessful = openFileAndGroup(1, fileName, partA, partF);
        if (!openSuccessful) return;
        readLookupTable(1);
        readTimestepList(1);
        readTimestepData(1);
    }

    public static void openInitFile(String fileName, String partA, String partF) {
        boolean openSuccessful = openFileAndGroup(2, fileName, partA, partF);
        if (!openSuccessful) return;
        readLookupTable(2);
        readTimestepList(2);
        readTimestepData(2);
    ...
    public static void closeSVFile() {
        closeFileAndGroup(1);
    }

    public static void closeInitFile() { closeFileAndGroup(2); }

    public static double[] readTimeSeriesData(String dssBPart, String partC, String units, String timeStep, Date startDate) {
        // Check if timeseries was in the HDF file
        int i = 0;
        boolean found = false;
        while (i<svLookupName.length && !found) {
            if (svLookupName[i].equals(dssBPart)) {
                if (svLookupTimestep[i].equals(timeStep.toLowerCase())) {
                    if(svLookupKind[i].equals(partC)) {
                        if (svLookupUnit[i].equals(units)) {
                            found = true;
                            break;
                        }
                    }
        ...
        // Identify the index of the timeseries data
        String[] listName = new String[0];
        String[] listKind = new String[0];
        double[][] data   = new double[0][0];
        startDate         = new Date(21, 9, 31, 24, 0);
        ...
        } else if (timeStep.toUpperCase().equals("1DAY")) {
    ...
    private static boolean openFileAndGroup(int flag, String fileName, String partA, String partF) {
        ...
        try {
            ...
            System.err.println("Failed to open HDF5 file " + fileName);
            return false;
        }
        ...
        if (fid<0) {
            ...
            try {
                gidPartA=HDF5Util.locateGroup(fid, partA);
                if (gidPartA>=0){
                    gidPartF = HDF5Util.locateGroup(gidPartA, partF);
                    if (gidPartF>=0){
                                ...
                                if (gidMonthly>=0 && tidAttr>=0 && sidAttr>=0){
                                    ...
                                    try{
                                        ...
                                        if (aidMonthly>=0){
                                            ...
                                            int month = TimeOperations.monthValue(monthlyDateStr.substring(2, 5));
                                ...
                                if (gidDaily>=0 && tidAttr>=0 && sidAttr>=0){
                                    ...
                                    try{
                                        ...
                                        if (aidDaily>=0){
                                            ...
                                            int month = TimeOperations.monthValue(dailyDateStr.substring(2, 5));
    ...
    private static void readLookupTable(int flag) {
    ...
    private static void readTimestepList(int flag) {
        readMonthlyTimestepList(flag);
        readDailyTimestepList(flag);
    }

    private static void readMonthlyTimestepList(int flag) {
        int gidMonthly=-1;
    ...
    private static void readDailyTimestepList(int flag) {
        int gidDaily=-1;
    ...
    private static void readTimestepData(int flag) {
        readMonthlyTimestepData(flag);
        readDailyTimestepData(flag);
    }

    private static void readMonthlyTimestepData(int flag) {
        int gidMonthly=-1;
    ...
    private static void readDailyTimestepData(int flag) {
        int gidDaily=-1;
    ...
    private static void closeFileAndGroup(int flag) {
        try {
            if (flag==1){
                if (svGidDaily>=0)
