...
public class ClpSolver {
	...
	public static void newProblem(String modelName, boolean useLpFile, StudyDataSet sds, int modelIndex){
		...
		for (int i=0; i<=1; i++){
			...
			while(constraintIterator.hasNext()){        
				...
				while(multIterator.hasNext()){
					...
					jClp.jarray_double_setitem(rowValue_array.get(rowCounter),j,multMap.get(multName).getValue().doubleValue());
					j++;
				}
