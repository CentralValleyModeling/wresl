...
import gov.ca.water.wrims.engine.core.components.*;
    ...
    private static Map<String, String> checkConfigFile(String configFilePath) {
        ...
        try {
            ...
            while(scanner.hasNextLine()) {
                ...
                if (line.contains(" ") && line.lastIndexOf(" ") + 1 < line.length() && line.length() >= 5) {
                    ...
                    if (key.equalsIgnoreCase("end") & value.equalsIgnoreCase("config")) {
                        break;
                    }
        ...
        String[] requiredFields = new String[]{"MainFile", "Solver", "DvarFile", "SvarFile", "SvarAPart", "SvarFPart", "InitFile", "InitFPart", "TimeStep", "StartYear", "StartMonth"};
        ...
        int bYr = Integer.parseInt((String)configMap.get("startyear"));
        int bMon = Integer.parseInt((String)configMap.get("startmonth"));
        if (configMap.keySet().contains("numberofsteps")) {
            int nsteps = Integer.parseInt((String)configMap.get("numberofsteps"));
            ...
            String[] endDateFields = new String[]{"StopYear", "StopMonth"};
        ...
        int endYr = Integer.parseInt((String)configMap.get("stopyear"));
        int endMon = Integer.parseInt((String)configMap.get("stopmonth"));
        int endday = TimeOperations.numberOfDays(endMon, endYr);
        if (!configMap.containsKey("stopday")) {
            configMap.put("stopday", Integer.toString(endday));
        }

        ControlData.defaultTimeStep = ((String)configMap.get("timestep")).toUpperCase();
        File mf = null;
        String mainfile = (String)configMap.get("mainfile");
        if (mainfile.contains(":")) {
