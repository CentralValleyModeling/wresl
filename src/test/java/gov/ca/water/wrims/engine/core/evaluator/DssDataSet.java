...
public class DssDataSet {
	...
	public void generateStudyStartIndex(){
		...
		if (TimeOperations.isMonthlyInterval(getTimeStep())){
	...
	public ArrayList<Double> getTimeseriesDataWithOptions(String selMon, Date selSd, Date selEd){
		...
		if (TimeOperations.isMonthlyInterval(timeStep)){
			dataDate= TimeOperations.backOneMonth(dataDate);
		}else{
			dataDate= TimeOperations.backOneDay(dataDate);
		}
		...
		for (int i=0; i<data.size(); i++){
			if (dataDate.after(selEd)){
				...
				if (TimeOperations.isMonthlyInterval(timeStep)){
					dataDate= TimeOperations.addOneMonth(dataDate);
				}else{
					dataDate= TimeOperations.addOneDay(dataDate);
				}
			}else{
				if (selMon.equalsIgnoreCase("all")){
					selData.add(data.get(i));
					selDataMap.put(sum, dataDate);
				}else if (TimeOperations.range(dataDate.getMonth()+1, m1, m2)){
					if (TimeOperations.range(dataDate.getMonth()+1, m1, m1)){
						sum=0;
						entryDate=dataDate;
					}
					sum=sum+data.get(i);
					if (TimeOperations.range(dataDate.getMonth()+1, m2, m2)){
						selData.add(sum);
