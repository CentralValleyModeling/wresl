...
public class VariableTimeStep {
	...
	public static void currTimeAddOneMonth(){
		...
		ControlData.currDay= TimeOperations.numberOfDays(ControlData.currMonth, ControlData.currYear);
	}
	...
	public static void procCycleTimeStep(StudyDataSet sds){
		...
		for (int i=0; i<timeStepList.size(); i++){
			...
			if (TimeOperations.isMonthlyInterval(timeStep) && ControlData.cycleTimeStepPriority<2.0){
	...
	public static void initialCurrTimeStep(List<String> modelList){
	...
	public static void setCurrentDate(StudyDataSet sds, int day, int month, int year){
		...
		if (TimeOperations.isMonthlyInterval(timeStep)){
			ControlData.currDay=TimeOperations.numberOfDays(month, year);
